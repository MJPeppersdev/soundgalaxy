...
var ParticleController = function (_VectorArrayObjectCon) {
	...
	_createClass(ParticleController, [{
		...
		value: function initRenderTarget(i) {
			var x = rand(this.bounds.x),
			    y = rand(this.bounds.y);

			this.renderTarget.setLife(0).setPosition(x, y).setLastPosition();

			var theta = this.canvas.origin.angleTo(this.renderTarget.position);
...
var AudioController = function () {
	...
	_createClass(AudioController, [{
		...
		value: function initAudio() {
			this.baseURL = "https://res.cloudinary.com/sf-cloudinary/video/upload/v1525614961/";
			this.currentFile = {};
			this.files = {};
			this.fileNames = ["beethovensymphony5.mp3", "chopinrevolutionary.mp3", "mountainking.mp3", "lisztliebestraum.mp3", "waltzflowers.mp3", "clairedelune.mp3", "schubertserenade.mp3"];
			this.trackTitles = ["Ludwig Van Beethoven - Symphony no. 5 mvt. 1", "Frederic Chopin - Etude op. 10 no. 9", "Edvard Grieg - In the Hall of the Mountain King", "Franz Liszt - Liebestraum", "Pyotr Tchaikovsky - Waltz of the Flowers", "Claude Debussy - Claire De Lune", "Franz Schubert - Schwanengesang (Swan Song) no. 4"];
...
var MPApp = function () {
	...
	_createClass(MPApp, [{
		...
		value: function drawBacklight(freqData) {
			...
			gradient.addColorStop(0, "hsla(" + hue + ", 75%, 70%, " + pow(avg / 128, 2) * 0.5 + ")");
		...
		value: function drawGlowLayer() {
			...
			this.canvas.ctx.filter = "blur(8px)";
			this.canvas.ctx.drawImage(this.canvas.frame, 0, 0);
